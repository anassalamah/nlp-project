...
if WRITE_FILE:
    ...
    if TEST:
        outfile_name = "wiki_features_test.csv"
    else:
        outfile_name = "wiki_features_train.csv"
        
    # Create File for combined scores
    outfile = open(outfile_name, 'w')
    o = DictWriter(outfile, ['Question ID','Sentence Position','Answer','Wikipedia Score','Wikipedia Found'], lineterminator='\n')
    o.writeheader()
...
for ii in joined_data:
    ...
    for key in guesses.keys():
        ...
        if wiki_found[key] == 1:
            ...
            nostop_text = nostop_dict[(ii['Question ID'],ii['Sentence Position'])]

            interest_word_count = 0
            for word in nostop_text.split(','):
                interest_word_count += 1
                #print "expression:", word
                if word not in ['\\','+2']:     # these regex cause program to crash
