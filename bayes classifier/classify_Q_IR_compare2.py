...
if __name__ == "__main__":
    ...
    for sp in [0,1,2,3,4]:
        for cat in ['science','lit','social','history']:
            Q_pred = Q_classifier.classify({'Sentence Position': str(sp),'category': cat})
            Q_prob_dist = Q_classifier.prob_classify({'Sentence Position': str(sp),'category': cat})
            IR_pred = IR_classifier.classify({'Sentence Position': str(sp),'category': cat})
            IR_prob_dist = IR_classifier.prob_classify({'Sentence Position': str(sp),'category': cat})
